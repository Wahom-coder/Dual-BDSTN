...
#  split dataset
x_train, y_train, x_test, y_test, x_pred, y_pred, adj_train, adj_test, adj_pred = split_data(df, adj, config.timestep,
...
for epoch in range(config.epochs):
    ...
    for data in train_bar:
        ...
        x_train_re = x_train.reshape(x_train.shape[0], x_train.shape[1], config.machine, -1)
        s_x_train = x_train_re[:, :, :, :2]
        t_x_train = (x_train_re[:, :, :, 2:]).reshape(x_train.shape[0], x_train.shape[1], -1)
        y_train_re = y_train.reshape(y_train.shape[0], config.machine, -1)
        s_y_train = (y_train_re[:, :, :2]).reshape(y_train.shape[0], -1)
        t_y_train = (y_train_re[:, :, 2:]).reshape(y_train.shape[0], -1)
        optimizer.zero_grad()
        y_train_pred = model(s_x_train, adj_train, t_x_train).to(torch.device("cpu"))
        loss = loss_function(y_train_pred.reshape(y_train_pred.shape[0], -1), s_y_train)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for data in test_bar:
            ...
            x_test_re = x_test.reshape(x_test.shape[0], x_test.shape[1], config.machine, -1)
            s_x_test = x_test_re[:, :, :, :2]
            t_x_test = (x_test_re[:, :, :, 2:]).reshape(x_test.shape[0], x_test.shape[1], -1)
            y_test_re = y_test.reshape(y_test.shape[0], config.machine, -1)
            s_y_test = (y_test_re[:, :, :2]).reshape(y_test.shape[0], -1)
            t_y_test = (y_test_re[:, :, 2:]).reshape(y_test.shape[0], -1)
            y_test_pred = model(s_x_test, adj_test, t_x_test).to(torch.device("cpu"))
            test_loss = loss_function(y_test_pred.reshape(y_test_pred.shape[0], -1), s_y_test)
            k = k + 1
...
# prediction
x_pred_tensor_re = x_pred_tensor.reshape(x_pred_tensor.shape[0], x_pred_tensor.shape[1], config.machine, -1)
s_x_pred_tensor = x_pred_tensor_re[:, :, :, :2]
t_x_pred_tensor = (x_pred_tensor_re[:, :, :, 2:]).reshape(x_pred_tensor.shape[0], x_pred_tensor.shape[1], -1)
y_pred_tensor_re = y_pred_tensor.reshape(y_pred_tensor.shape[0], config.machine, -1)
s_y_pred_tensor = (y_pred_tensor_re[:, :, :2]).reshape(y_pred_tensor.shape[0], -1)
t_y_pred_tensor = (y_pred_tensor_re[:, :, 2:]).reshape(y_pred_tensor.shape[0], -1)
y_pred_pred = model(s_x_pred_tensor, adj_pred_tensor, t_x_pred_tensor).to(torch.device("cpu"))

# Pad with zeros to match the initial number of features (same to scaler.fit_transform) for inverse transform
y_pred_pred = y_pred_pred.reshape(y_pred_pred.shape[0], -1)
pad = np.zeros((y_pred_pred.shape[0], x_pred_tensor.shape[2] - y_pred_pred.shape[1]))
y_pred_pred = np.hstack((y_pred_pred.detach().numpy(), pad))
y_pred_pred = scaler.inverse_transform(y_pred_pred)
y_pred_tensor = scaler.inverse_transform(y_pred_tensor.detach().numpy())

# select the indicator labels
y_pred_pred = y_pred_pred[:, :x_pred_tensor.shape[2] - pad.shape[1]]
y_pred_tensor = y_pred_tensor[:, :x_pred_tensor.shape[2] - pad.shape[1]]
y_pred_pred[y_pred_pred < 0] = 0
# show the last 20 step (your selected labels)
# plt.figure(figsize=(16, 6))
plt.title(config.model_name)
plt.plot(range(0, 20), y_pred_pred[-20:, 2], 'r', label='a_pred', marker='.')
plt.plot(range(0, 20), y_pred_tensor[-20:, 2], 'b', label='a_tst', marker='*')
plt.show()

# MAE and RMSE
print(metrics.mean_absolute_error(y_pred_pred, y_pred_tensor))
print(np.sqrt(metrics.mean_squared_error(y_pred_pred, y_pred_tensor)))
