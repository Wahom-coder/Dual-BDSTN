...
class Dual_BDSTN(nn.Module):
    def __init__(self, S_inputsize, T_inputsize, nhid, outputsize, machine_num, len, dropout, num_layers, nhead):
        ...
        self.out = nn.Linear(nhid * self.machine_num, nhid)
        self.gru_emb = self.gru = nn.GRU(T_inputsize, nhid, num_layers, batch_first=True)
        # Self-Attention
        ...
        self.self_layernorm = nn.LayerNorm([len, nhid])
        # MultiheadAttention
        ...
        self.m_layernorm = nn.LayerNorm([len, nhid])

        self.gru = self.gru = nn.GRU(2 * nhid, nhid, num_layers, batch_first=True)
    ...
    def forward(self, s, adj, t, mask=None):
        ...
        Q_t = Q_t.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        K_t = K_t.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        V_t = V_t.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        ...
        t_output = t_output.view(batch_size, len, self.n_heads * (self.hid_dim // self.n_heads))
        ...
        s_out = self.out(s_out)
        Q_s = self.m_w_q(t_out)
        K_s = self.m_w_k(s_out)
        V_s = self.m_w_v(s_out)
        Q_s = Q_s.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        K_s = K_s.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        V_s = V_s.view(batch_size, len, self.n_heads, self.hid_dim // self.n_heads).permute(0, 2, 1, 3)
        ...
        s_output = s_output.view(batch_size, len, self.n_heads * (self.hid_dim // self.n_heads))
        s_output = self.m_layernorm(self.m_fc(s_output)+s_out)

        # Concat
        output = torch.cat((t_output, s_output), dim=-1)
        # gru
...
if __name__ == "__main__":
   batch_size, len, node, d = 64, 10, 13, 2
   nhid = 64
   nhead = 8
   dropout = 0.2
   num_layers = 1
   ...
   t = torch.randn(batch_size, len, node * d)
